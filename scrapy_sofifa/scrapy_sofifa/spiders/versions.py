...
class VersionsSpider(Spider):
    ...
    def parse(self, response):
        ...
        for (link, release_date) in _get_list_of_updates(current_page_main_version, response):
            yield {
                'version_name': current_page_main_version[1],
                'version_id': utils.get_id_from_version_link(link),
                'release_date': _get_release_date(release_date)
            }
...
def _get_list_of_updates(current_page_main_version, response):
    ...
    return _remove_buggy_last_fifa_20_version(list_of_updates) \
        if current_page_main_version[1] == 'FIFA 20' \
        else list_of_updates
...
def _get_release_date(sofifa_date):
